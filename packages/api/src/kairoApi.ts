...
// In-flight AI request deduplication cache to eliminate duplicate concurrent inference calls
const inFlightRequests = new Map<string, Promise<any>>();

async function deduplicatedAiCall<T>(key: string, fn: () => Promise<T>): Promise<T> {
...
export const kairoApi = {
  async sendMessage(
    ...
    const dedupKey = `chat:${message.trim()}:${JSON.stringify(clientContext || {})}`;
    return deduplicatedAiCall(dedupKey, async () => {
      try {
  ...
  async extractBrainDump(transcript: string): Promise<BrainDumpExtractResponse> {
    const dedupKey = `braindump:${transcript.trim()}`;
    return deduplicatedAiCall(dedupKey, async () => {
      try {
  ...
  async decomposeGoal(
    ...
    const dedupKey = `goal:${goalTitle}:${targetDate}:${category}`;
    return deduplicatedAiCall(dedupKey, async () => {
      try {
